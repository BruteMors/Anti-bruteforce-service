...
func TestBlackList_AddIP(t *testing.T) {
	...
	mockStore := mock_service.NewMockBlackListStore(controller)
	...
	blackListService := service.NewBlackList(mockStore, logger)
	blacklist := NewBlackList(blackListService, logger)

	// Создаем тестовый HTTP-сервер
	router := httprouter.New()
	router.POST("/auth/blacklist", blacklist.AddIP)

	// Создаем тестовый IP для добавления в черный список
	ip := cases[0].network
	...
	// Отправляем POST-запрос на сервер для добавления IP в черный список
	...
	// Попытка добавления уже существующего IP в черный список
	req, err = http.NewRequest("POST", "/auth/blacklist", bytes.NewReader(body))
	require.NoError(t, err)
	req.Header.Set("Content-Type", "application/json")
	rr = httptest.NewRecorder()
...
func TestBlackList_RemoveIP(t *testing.T) {
	// Создаем экземпляр объекта BlackList
	logger := zap.NewExample().Sugar()
	...
	// Создаем тестовый HTTP-сервер
	router := httprouter.New()
	router.DELETE("/auth/blacklist", blacklist.RemoveIP)

	// Создаем тестовый IP для удаления из черного списка
	ip := cases[0].network
...
func TestBlackList_ShowIPList(t *testing.T) {
	// Создаем экземпляр объекта BlackList
	logger := zap.NewExample().Sugar()
	...
	cases := []entity.IpNetwork{entity.IpNetwork{
		Ip:   "192.168.1.1",
		Mask: "255.255.255.0",
	}, entity.IpNetwork{
		Ip:   "192.168.2.1",
	...
	// Создаем тестовый HTTP-сервер
	r := httprouter.New()
	r.GET("/auth/blacklist", blacklist.ShowIPList)
	ts := httptest.NewServer(r)
	defer ts.Close()

	// Отправляем GET-запрос к тестовому серверу
	res, err := http.Get(ts.URL + "/auth/blacklist")
	require.NoError(t, err)
